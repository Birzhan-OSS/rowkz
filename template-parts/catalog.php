...
$lead      = isset( $args['lead'] ) ? $args['lead'] : 'Официальные поставки, консультация по подбору и сервис в Казахстане.';
	...
	<section class="rk-catalog">
		<div class="container">
			<form id="catalog-filter" class="rk-filters" data-root="<?php echo esc_attr( $root_slug ); ?>" role="search">
				...
				<?php if ( $subcats ) : ?>
					<div class="rk-chips" role="group" aria-label="Подрубрики">
						<button type="button" class="rk-chip is-active" data-subcat="" aria-pressed="true">Все</button>
				...
				<div class="row g-2">
					<div class="col-md">
						<input type="search" name="s" class="form-control" placeholder="Поиск по названию или характеристикам" aria-label="Поиск">
					</div>
					<?php if ( ! is_wp_error( $proizvoditeli ) && count( $proizvoditeli ) > 1 ) : ?>
						<div class="col-md-3">
							<select name="proizvoditel" class="form-select" aria-label="Производитель">
					...
					<?php if ( ! is_wp_error( $materials ) && $materials ) : ?>
						<div class="col-md-3">
							<select name="material" class="form-select" aria-label="Материал">
			...
			<div id="catalog-results" class="row rk-grid">
				<div class="col-12 rk-empty"><p>Загрузка товаров…</p></div>
			</div>
		</div>
	</section>
</main>
...
document.addEventListener('DOMContentLoaded', function () {
	...
	function fetchCatalog() {
		...
		fetch(<?php echo wp_json_encode( admin_url( 'admin-ajax.php' ) ); ?>, { method: 'POST', credentials: 'same-origin', body: body })
			...
			.catch(() => { results.innerHTML = '<div class="col-12 rk-empty"><p>Не удалось загрузить товары. Обновите страницу.</p></div>'; results.style.opacity = ''; });
	}
